    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsList = details.participants.length > 0
          ? `<ul class="participants-list">${details.participants.map(p => `<li data-activity="${name}" data-email="${p}"><span>${p}</span><button class="delete-btn" type="button" title="Unregister">✕</button></li>`).join('')}</ul>`
